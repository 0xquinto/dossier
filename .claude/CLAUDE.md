...
Scout-1 calls the `board-aggregator` CLI (installed in `.venv/`) which scrapes 14 boards:
- python-jobspy: Indeed, LinkedIn
- Custom scrapers: Himalayas, We Work Remotely, HN Who's Hiring, HN Freelancer, CryptoJobsList, crypto.jobs, web3.career, CryptocurrencyJobs, RemoteOK, Reddit, Indie Hackers, No Code Jobs, 80,000 Hours
...
**Structure**: `.claude/agents/` (12 agent defs), `board_aggregator/` (14 scrapers, 14 boards), `tests/` (mocked HTTP), `scripts/` (tracker.py, generate-pdf.mjs, normalize-ats.mjs), `dashboard/` (Go TUI)
...
- **Definition — "successfully fetched source":** a source is successfully fetched ONLY when you retrieved its full page body with a fetch/crawl tool (`WebFetch` or `mcp__exa__web_fetch_exa`) and that body returned readable content containing the field verbatim. A search-result snippet, title, preview, summary, or any excerpt returned by a search tool (`WebSearch`, `mcp__exa__web_search_exa`, `mcp__exa__web_search_advanced_exa`) is NOT a fetched source: it is a lead to be fetched. A field that appears only in a search snippet and was never confirmed in a fetched page body is at most `unverifiable` — it may NEVER be labeled `verified`/`confirmed`. To label a field `verified` you must (a) have fetched the page and (b) cite that fetched page's URL.
