...
Confidence applies to every claim, not emails alone. Never write "confirmed" for a title, role, or action (e.g. "personally posted the job") unless a fetched source states it. If the evidence is weaker, say `inferred` or `unverifiable` and describe what the source actually showed.
